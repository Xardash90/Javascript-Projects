	...
	ECHO '<div class="row container d-flex justify-content-center">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h4 class="panel-title" title=""> <i style="color: #66CDAA;" class="fa fa-check"></i> Einträge im Status "Abgeschlossen"</h4>
					</div>
				';
		//Funktionen für TOP Button --> Datum, Mitarbeiter auswählen und Alle anzeigen

		$query = "SELECT * FROM `mp_38matrix` WHERE `mp_5` = 'Abgeschlossen' AND `mp_1` = '".$_SESSION["location"]."' AND `mp_9` = 'CC_SCHIFTREPORT' AND DATE(`mp_2`) = CURDATE()  ORDER BY `mp_2` DESC"; 
...
ECHO '
						<div class="panel-body">
							...
							<span class="pull-right"><small>Abgeschlossen: '.$row['mp_6'].' </small></br><small>Am: <i class="fa fa-calendar" aria-hidden="true"></i>'.date(' d.m.Y'  , strtotime($row["mp_16"])).' <i class="fa fa-clock-o" aria-hidden="true"></i>'.date(' H:i:s'  , strtotime($row["mp_16"])).' Uhr</small> 
							<br><small>Bereich: '.$row["mp_7"].'</small>
							</span>								
						</div>
						<div class="panel-footer">
						  <small>Eintrag existiert: '.$ergebniss->format('%dd')." ".$ergebniss->format('%hh')." ".$ergebniss->format('%im')." ".$ergebniss->format('%ss')."".'</small>
